...
for me in sizes:
    ...
    for reg in regions:
        ...
        if holes:
            ...
            p, tt = CloudGen.CreateCloud_2(x, y, (2/3)*me) 

        # Get the name to save the information.
        if holes:
            folder = 'Holes/' + size + '/'                                      # Define the directory.
        else:
            folder = 'Clouds/' + size + '/'                                     # Define the directory.
        ...
        pd.DataFrame(p).to_csv(f'{folder}{reg}_p_2.csv', index = False, header = False)
        pd.DataFrame(tt).to_csv(f'{folder}{reg}_tt_2.csv', index = False, header = False)